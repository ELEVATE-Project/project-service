...
module.exports = class ProjectTemplateTask {
	...
	static taskDocuments(filterData = 'all', fieldsArray = 'all', skipFields = 'none') {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let templateTasks = await database.models.projectTemplateTasks.find(queryObject, projection).lean()
	...
	static updateTaskDocument(query = {}, updateObject = {}) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				let updateResponse = await database.models.projectTemplateTasks.updateOne(query, updateObject)
	...
	static findOneAndUpdate(findQuery, UpdateObject, returnData = {}) {
		return new Promise(async (resolve, reject) => {
			try {
				let templateTask = await database.models.projectTemplateTasks.findOneAndUpdate(
					findQuery,
					UpdateObject,
					returnData
				)
	...
	/**
	 * Delete projectTemplateTasks documents based on the provided MongoDB filter.
	 *
	 ...
	 */
	static deleteProjectTemplateTasks(filter) {
		return new Promise(async (resolve, reject) => {
			try {
				let deleteDocuments = await database.models.projectTemplateTasks.deleteMany(filter)
