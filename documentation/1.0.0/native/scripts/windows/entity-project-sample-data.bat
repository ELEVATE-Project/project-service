...
for /f "delims=" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "var result=db.getSiblingDB('%ENTITY_SERVICE_DB_NAME%').%ENTITY_TYPE_COLLECTION%.insertOne({\"name\": \"state\", \"toBeMappedToParentEntities\": true, \"immediateChildrenEntityType\": [\"district\"], \"isDeleted\": false}); result.insertedId;"') do set ENTITY_TYPE_ID=%%i
...
for /f "delims=" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "var result=db.getSiblingDB('%ENTITY_SERVICE_DB_NAME%').%ENTITY_TYPE_COLLECTION%.insertOne({\"name\": \"district\", \"toBeMappedToParentEntities\": true, \"immediateChildrenEntityType\": [\"block\"], \"isDeleted\": false}); result.insertedId;"') do set ENTITY_TYPE_DISTRICT_ID=%%i
...
for /f "delims=" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "!mongo_query!"') do set "ENTITY_ID=%%i"
...
for /f "delims=" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "!mongo_query!"') do set "ENTITY_DISTRICT_ID=%%i"
...
rem Execute the MongoDB query
mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "%mongo_update_query%"
...
for /f "delims=" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "!mongo_update_query!"') do set "USER_EXTENSION_ID=%%i"
...
for /f "tokens=*" %%i in ('mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "!mongo_query!"') do set "PROGRAM_ID=%%i"
...
for /f "tokens=*" %%i in (
    'mongo --host %MONGO_HOST% --port %MONGO_PORT% --quiet --eval "var doc = %SOLUTION_DOCUMENT%; var result = db.getSiblingDB('%PROJECT_DB_NAME%').%SOLUTIONS_COLLECTION%.insertMany(doc); if (result.insertedIds && Object.keys(result.insertedIds).length > 0) { print(result.insertedIds); } else { throw new Error('Insert failed'); }"'
...
for /f "delims=" %%i in ('mongo --host "%MONGO_HOST%" --port "%MONGO_PORT%" --quiet --eval "var doc = %CONFIGURATIONS_DOCUMENT%; var result = db.getSiblingDB('%PROJECT_DB_NAME%').%CONFIGURATIONS_COLLECTION%.insertOne(doc); if (result.insertedId) { print(result.insertedId); } else { throw new Error('Insert failed'); }"') do set "CONFIGURATION_ID=%%i"
