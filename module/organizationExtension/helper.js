...
const projectTemplateQueries = require(DB_QUERY_BASE_PATH + '/projectTemplates.js')
...
module.exports = class OrganizationHelper {
	...
	/**
	 * Updates relatedOrgs in parent project-templates
	 *
	 * @method
	 * @name updateRelatedOrgs
	 * @param {Object} request - requestData
	 *
	 * @returns {Promise<Object>} - Returns success response or error information.
	 */
	static updateRelatedOrgs(request) {
		return new Promise(async (resolve, reject) => {
			try {
				// extract tenantId and orgId from request
				// if userDetails exist, take from there, else fallback to request body
				const tenantId = request.userDetails?.tenantAndOrgInfo
					? request.userDetails.tenantAndOrgInfo.tenantId
					: request.body.tenant_code
				const orgId = request.userDetails?.tenantAndOrgInfo
					? request.userDetails.tenantAndOrgInfo.orgId[0]
					: request.body.code
				...
				// update all project templates matching tenantId, orgId and reusable flag
				// set their visibleToOrganizations field with the new relatedOrgCodes
				const updatedRelatedOrgs = await projectTemplateQueries.updateMany(
					{
						tenantId,
						orgId,
						isReusable: true,
					...
					{
						$set: {
							visibleToOrganizations: relatedOrgCodes,
						},
					}
				)

				// check if update was not acknowledged or failed
				if (!updatedRelatedOrgs && !updatedRelatedOrgs.acknowledged) {
					throw {
						success: false,
						status: HTTP_STATUS_CODE.bad_request.status,
						message: CONSTANTS.apiResponses.RELATED_ORGS_UPDATE_FAILED,
					}
				}

				// success response with how many templates matched and how many were updated
				return resolve({
					success: true,
					status: HTTP_STATUS_CODE.ok.status,
					message: CONSTANTS.apiResponses.RELATED_ORGS_UPDATED_SUCCESSFULLY,
					result: {
						templatesFound: updatedRelatedOrgs.n,
						templatesUpdated: updatedRelatedOrgs.nModified,
					},
				})
			} catch (error) {
				return reject({
